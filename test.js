...
var ASM_CONSTS = {
  ...
  var my_test = {sampler:undefined};
  function _my_bind_group(deviceId, pipelineId) {
          const device = WebGPU.mgrDevice.get(deviceId);
          const pipeline = WebGPU.mgrRenderPipeline.get(pipelineId);
          // alert("device:" + device);
          // alert("pipeline:" + pipeline);
          // alert("layout:" + pipeline.getBindGroupLayout(0));
  
          if (my_test.sampler === undefined) {
              console.log("create");
              my_test.sampler = device.createSampler({
                  magFilter: 'linear',
                  minFilter: 'linear',
              });
          }
...
unexportedRuntimeSymbol('WebGPU', false);
unexportedRuntimeSymbol('JsValStore', false);
unexportedRuntimeSymbol('ALLOC_NORMAL', false);
unexportedRuntimeSymbol('ALLOC_STACK', false);
unexportedRuntimeSymbol('allocate', false);
unexportedRuntimeSymbol('my_test', false);
